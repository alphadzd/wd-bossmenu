...
local function DebugPrint(msg)
    if DEBUG_MODE then
        print("^3[wd-bossmenu Debug]^7 " .. msg)
        LAST_DEBUG = msg
    end
end
...
RegisterNetEvent('wd-bossmenu:client:JobChanged', function(jobName)
    if menuOpen and PlayerData.job and PlayerData.job.isboss and PlayerData.job.name == jobName then
        QBCore.Functions.TriggerCallback('wd-bossmenu:server:GetJobData', function(jobData)
            if jobData then
                SendNUIMessage({
                    action = "refreshData",
                    jobData = jobData
        ...
        if Config.TargetSystem == "qb-target" then
            exports['qb-target']:AddBoxZone("job_application_"..jobName, location.coords, location.length, location.width, {
                ...
                options = {
                    {
                        type = "client",
                        event = "wd-bossmenu:client:OpenApplicationForm",
                ...
                options = {
                    {
                        ...
                        label = location.label,
                        onSelect = function()
                            TriggerEvent("wd-bossmenu:client:OpenApplicationForm", {jobData = jobName})
                        end,
                        distance = 2.0
                    }
                }
...
RegisterNetEvent('wd-bossmenu:client:OpenApplicationForm', function(data)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:CheckPendingApplication', function(hasPendingApplication)
...
RegisterNetEvent('wd-bossmenu:client:SyncPermissions', function(targetCitizenid, permissions)
...
RegisterNUICallback('getApplications', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:GetJobApplications', function(applications)
        cb(applications)
    end, jobName)
end)
...
RegisterNUICallback('updateApplicationStatus', function(data, cb)
    TriggerServerEvent('wd-bossmenu:server:UpdateApplicationStatus', data.applicationId, data.status, data.notes)
    cb('ok')
end)
            ...
            if Config.TargetSystem == "qb-target" then
                exports['qb-target']:AddBoxZone(zoneName, location.coords, location.length, location.width, {
                    ...
                    options = {
                        {
                            type = "client",
                            event = "wd-bossmenu:client:TriggerOpenManager", 
                    ...
                    options = {
                        {
                            ...
                            label = "Manage " .. jobLabel,
                            onSelect = function()
                                TriggerEvent("wd-bossmenu:client:TriggerOpenManager", {jobData = jobName})
                            end,
                            canInteract = function()
                                if PlayerData.job and PlayerData.job.name == jobName then
                                    if PlayerData.job.isboss then
...
RegisterNetEvent('wd-bossmenu:client:TriggerOpenManager', function(data)
    TriggerServerEvent('wd-bossmenu:server:RequestRefreshJobData')
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:HasJobAccess', function(hasAccess) 
...
RegisterNUICallback('updateEmployeePermissions', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:UpdateEmployeePermissions', function(result)
        cb(result) 
...
RegisterNUICallback('getEmployeePermissions', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:GetEmployeePermissions', function(permissions)
        cb(permissions)
    end, citizenid, jobName)
end)

RegisterNetEvent('wd-bossmenu:client:RefreshPermissions', function(permissions)
    ...
    if permissions then
        ...
        QBCore.Functions.TriggerCallback('wd-bossmenu:server:GetEmployeePermissions', function(newPermissions)
...
function OpenJobManager(jobName)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:GetJobData', function(jobData)
        ...
        QBCore.Functions.TriggerCallback('wd-bossmenu:server:GetSettings', function(settings)
            if not settings then
                return
            end
            QBCore.Functions.TriggerCallback('wd-bossmenu:server:GetSocietyData', function(societyData)
...
RegisterNetEvent('wd-bossmenu:client:OpenManager', function()
    DebugPrint("Blocked automatic opening from original event")
end)

-- Refresh data event handler
RegisterNetEvent('wd-bossmenu:client:RefreshData', function()
    ...
    local menuOptions = {
        {
            ...
            params = {
                event = "wd-bossmenu:client:FireEmployeeConfirmed",
                args = {
                    citizenid = citizenid,
                    confirmed = true
                }
...
RegisterNUICallback("submitApplication", function(data, cb) 
    SetNuiFocus(false, false)
    TriggerServerEvent("wd-bossmenu:server:SubmitApplication", data.jobName, data.answers)
end)
...
RegisterNetEvent('wd-bossmenu:client:FireEmployeeConfirmed', function(data)
    SendNUIMessage({
        action = "fireEmployeeResponse",
        confirmed = data.confirmed,
        citizenid = data.citizenid
...
RegisterNUICallback('updateEmployee', function(data, cb)
    TriggerServerEvent('wd-bossmenu:server:UpdateEmployee', data.citizenid, PlayerData.job.name, data.grade)
    cb('ok')
end)

RegisterNUICallback('removeEmployee', function(data, cb)
    TriggerServerEvent('wd-bossmenu:server:RemoveEmployee', data.citizenid, PlayerData.job.name)
    cb('ok')
end)

RegisterNUICallback('saveSettings', function(data, cb)
    TriggerServerEvent('wd-bossmenu:server:SaveSettings', data)
    cb('ok')
end)
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    if Config.TargetSystem ~= "qb-target" and Config.TargetSystem ~= "ox_target" then
        print("^1[wd-bossmenu]^7 Invalid target system in config: " .. (Config.TargetSystem or "nil") .. ". Defaulting to qb-target.")
...
CreateThread(function()
    while true do
        ...
        if menuOpen then
            TriggerServerEvent('wd-bossmenu:server:RefreshPlayTime')
        end
...
RegisterNUICallback('depositMoney', function(data, cb)
    ...
    TriggerServerEvent('wd-bossmenu:server:DepositMoney', data.amount, data.note, data.jobName)
    cb({success = true})
end)
...
RegisterNUICallback('withdrawMoney', function(data, cb)
    ...
    TriggerServerEvent('wd-bossmenu:server:WithdrawMoney', data.amount, data.note, data.jobName)
    cb({success = true})
end)
...
RegisterNUICallback('transferMoney', function(data, cb)
    ...
    TriggerServerEvent('wd-bossmenu:server:TransferMoney', data.citizenid, data.amount, data.note, data.jobName)
    cb({success = true})
end)
...
RegisterNetEvent('wd-bossmenu:client:OpenApplicationForm', function(data)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:CheckApplicationStatus', function(result)
...
RegisterNUICallback('getPlaytimeData', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:GetPlaytimeData', function(playtimeData)
...
RegisterNUICallback('hireEmployee', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:HireNewEmployee', function(result)
        cb(result)
...
RegisterNUICallback('updateJobGrade', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:UpdateJobGrade', function(result)
        cb(result)
...
RegisterNUICallback('addJobGrade', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:AddJobGrade', function(result)
        cb(result)
...
RegisterNUICallback('deleteJobGrade', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:DeleteJobGrade', function(result)
        cb(result)
...
RegisterNUICallback('getJobGrades', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('wd-bossmenu:server:GetJobGrades', function(gradesData)
        if gradesData then
            cb(gradesData)
        else
            cb(false)
