...
function playSound(soundType) {
    // For police application, only use click sound regardless of the requested sound type
    const clickSound = document.getElementById('clickSound');
    
    if (clickSound) {
        clickSound.currentTime = 0;
        clickSound.volume = 0.3; // Reduced volume
...
function setupSoundEffects() {
    // Add click sounds to all buttons - only use click sound for police application
    document.querySelectorAll('.btn, .nav-item, .tab, button').forEach(element => {
    ...
    // Add click sounds to application buttons
    document.querySelectorAll('#cancel_app, #finish_task').forEach(element => {
        element.addEventListener('click', function() {
            playSound('click');
        });
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setupSoundEffects(); // Add sound effects
...
document.addEventListener('keydown', function(event) {
    if (event.keyCode === 27) {
        ...
        // Check if application form is open
        const applicationContainer = document.getElementById('application-container');
        if (applicationContainer && applicationContainer.style.display === 'block') {
            closeUIAndHideBackground();
            return;
        }
        ...
        if (document.querySelectorAll('.modal.show').length === 0) {
            // Use our centralized function to close UI and hide background
            closeUIAndHideBackground();
        }
    }
});
...
window.addEventListener('message', function(event) {
    ...
    if (data.action === 'openUI') {
        // Show the body when UI is opened
        ...
        // Immediately hide the body to fix background visibility issue
        document.body.classList.remove('visible');
        document.body.style.display = 'none';
    ...
    } else if (data.action === 'updatePermissionToggles') {
...
const openApplication = (data) => {
    ...
    // Make sure body is visible when opening application
    document.body.style.display = 'flex';
    document.body.classList.add('visible');
    ...
    // Make sure the application container is visible and on top
    const appContainer = document.getElementById('application-container');
    appContainer.style.display = "block";
    appContainer.style.zIndex = "1000";
    
    // Hide the main container to avoid conflicts
    document.getElementById('container').style.display = "none";
    
    // Play a click sound when the form is opened
    playSound('click');
};
...
function GetParentResourceName() {
    try {
        ...
        return 'wd-bossmenu'; // Default for testing in browser
    }
}
...
// Function to properly close UI and hide background
function closeUIAndHideBackground() {
    // Play click sound
    playSound('click');
    ...
    // Hide the body and remove visible class
    document.body.classList.remove('visible');
    document.body.style.display = 'none';
